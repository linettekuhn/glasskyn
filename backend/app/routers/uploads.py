...
async def process_multi_images(
    ...
    # Determine product name, brand, and type (LLM-first, OBF fallback)
    product_name = None
    brand = None
    product_type = None
    ...
    if product_name or brand:
        ...
        logger.info("LLM extracted: name=%s brand=%s", product_name, brand)
    elif body.barcode:
        logger.info("LLM returned nulls, trying barcode lookup")
        try:
            ...
            if obf_result:
                ...
                logger.info("Barcode miss, leaving fields blank")
        except Exception as e:
