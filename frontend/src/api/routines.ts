...
export async function listRoutines(routineType = 'skincare'): Promise<Routine[]> {
  const response = await apiClient.get('/routines', { params: { routine_type: routineType } });
  return response.data;
}

export async function getActiveRoutine(routineType = 'skincare'): Promise<Routine> {
  const response = await apiClient.get('/routines/active', { params: { routine_type: routineType } });
  return response.data;
}
...
export async function updateRoutine(id: number, data: {
  name?: string;
  is_active?: boolean;
  steps?: {
    step_order: number;
...
export async function updateRoutineStep(routineId: number, stepId: number, data: {
  step_order?: number;
  product_id?: number | null;
  step_type?: string;
  time_of_day?: string;
...
export async function markStepComplete(routineId: number, stepId: number): Promise<void> {
  await apiClient.patch(`/routines/${routineId}/steps/${stepId}/complete`);
}

export async function listTemplates(routineType = 'skincare', skinType?: string, concern?: string): Promise<RoutineTemplate[]> {
...
export async function cloneTemplate(templateId: number, name?: string): Promise<Routine> {
  const response = await apiClient.post('/routines/clone', { template_id: templateId, name });
  return response.data;
}
